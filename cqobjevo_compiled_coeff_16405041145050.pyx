...
cdef class CompiledStrCoeff(StrCoeff):
    ...
    cdef void _call_core(self, double t, complex * coeff):
        ...
        coeff[1] = 1j/4*(- dwQ*exp(-0.5*(t/dtQ)**2) * t/(dtQ**2) + (2*w0*dwP*cos(2*w0*t) if t > delay and t < delay+dtP else 0))/(w0 + dwQ*exp(-0.5*(t/dtQ)**2) + (dwP*sin(2*w0*(t-delay)) if t > delay and t < delay+dtP else 0))
